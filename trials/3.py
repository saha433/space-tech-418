...
SAFE_OFF_NADIR_MARGIN_DEG = 5.0
HOLD_PAD_S = 0.30
MAX_SLEW_RATE_DPS = 2.5
MIN_SHOT_SPACING_S = 2.6
TIME_GRID_S = 1.0
TARGET_GRID_N = 10
EVAL_GRID_N = 13
MAX_SHOTS = 8
...
TARGET_GRID_N_HARD = 10
EVAL_GRID_N_HARD = 21
UNIFIED_CANDIDATES_PER_TARGET = 3
...
def _build_attitude_schedule(shots: Sequence[ShotCandidate], T: float, integration_s: float) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    attitude_dicts = [{"t": round(t, 4), "q_BN": [float(x) for x in q]} for t, q in cleaned]
...
def _sequence_proxy(shots: Sequence[ShotCandidate], cell_count: int, T: float, integration_s: float) -> float:
    ...
    time_cost = (0.015 + 0.055 * avg_w) * span / max(T, 1e-9)
    shot_cost = (0.010 + 0.018 * avg_w) * len(shots)
    slew_cost = (0.0007 + 0.0010 * avg_w) * total_slew
...
def plan_imaging(
    ...
    max_shots = int(round(MAX_SHOTS - 6.0 * avg_weight))
