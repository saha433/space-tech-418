...
- `PROBLEM_STATEMENT.pdf`  
  Official hackathon problem statement for the Lost in Space / Earth-observation scheduling track.
...
If Basilisk is installed, the organizer-style harness can be run with:

```bash
python3 Lost-In-Space/organizer_harness/run_evaluation.py \
  --submission "space-tech/Space Tech Aeon/optimised_submission_final.py" \
  --all
```

For mock mode through the organizer harness:
...
> We solve the task as a coverage-first stop-and-stare beam search: generate feasible AOI pointings, project their footprints, and select a compact high-coverage mosaic while holding attitude fixed during each exposure to avoid smear.
